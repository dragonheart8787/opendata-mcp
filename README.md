...
**3. 錯誤診斷日誌（僅在失敗時，且不含身分）**

在**正常成功的請求路徑上，查詢內容完全不會被寫進任何日誌**。

唯一的例外是環境部（MOENV）上游請求失敗時：adapter 會寫一行診斷訊息到 Cloudflare Workers Logs，內容包含請求 URL（**API 金鑰已遮蔽**，但 URL 裡會帶到查詢參數，例如縣市名稱）與上游回應內容的前 500 個字元，用途純粹是排查上游壞掉的原因。這行日誌**同樣不含任何請求方身分資訊**（沒有 IP、沒有使用者識別）。日誌保存期限依 Cloudflare Workers Logs 的預設設定，我們不另外匯出或保留副本。
...
此外，**僅限用瀏覽器造訪介紹頁面時**（不影響 MCP 工具呼叫）：該頁面會載入 Google Fonts（`fonts.googleapis.com`／`fonts.gstatic.com`）以及 Three.js 的 CDN（`cdn.jsdelivr.net`，備援為 `esm.sh`／`unpkg.com`）來呈現字體與 3D 背景。載入這些外部資源時，你的瀏覽器會直接連線到這些 CDN，因此**它們會看到你的 IP 與瀏覽器 User-Agent**。這是載入任何第三方 CDN 資源的固有行為。介紹頁面本身**不設定任何 cookie、不使用 localStorage、也不發送任何 API 請求**（頁面上的示範內容是寫死的靜態範例，不是即時查詢）。
